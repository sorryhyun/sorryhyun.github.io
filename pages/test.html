...
<details><summary>상세 및 개인 의견</summary>
  ...
  **논문의 구체적인 방법론** : corrupted(perturbation, e~N(0,v)) input에 대한 output과 정상적인 input에 대한 output을 attention과 MLP를 분리하여 비교한 결과, MLP는 layer 중반, attention은 레이어 후반부에서 추론에 대한 지배적인 영향을 발휘합니다. 또한 15번 레이어(GPT2-XL 모델에서 clean run과 corrupted run의 출력을 비교했을 때, h가 가장 많이 바뀌는 레이어)의 {subject 단어가 포함된 텍스트들을 입력할 때 last token of subject 부분의 k의 평균, 출력을 최대로 바꾸는 MLP부분} 을 수정함으로서 모델에 존재하는 지식(s, r, o)을 변경하는(edit) 방법을 제안했는데, [후속 연구](https://docs.google.com/document/d/1c8D1mw063tHBR3QZicxjZceH-aLZZnl23xvwDxBXeek/edit?pli=1#heading=h.pj59vjix7yvl)에서는 이 방법이 효과적이지 않은 것으로 나왔습니다.
...
**기존 문제** : 명령문이나 질의응답 과정 등은 ‘자연스러운 문장의 일부’는 아닙니다. 즉 소설과 같이 자연스러운 언어 흐름이 아닌 것입니다. 그러므로 ‘다음 토큰을 예측하는 작업’으로 학습된 LLM은 ICL을 수행하게끔 하는 prompt가 부자연스럽다고(low confidence) 여깁니다. 그럼에도 LLM은 사용자의 지시문과 보기 지문 등을 잘 구분할 수 있습니다. 이에 대해 명확한 원리를 파악하기 어려웠습니다.

**논문이 이끌어낸 직관** :

LLM은 특정 지시문이 입력되거나 보기 문단이 입력되는 상황을 포착하여, 어떤 가중치의 지식을 사용해야 할 지 감별할 수 있습니다. 즉 주어진 입력에 대해 ICL이 필요한 구간을 이해합니다. 또한 다양한 prompt/concept로 학습되지 않은 경우 ICL을 잘 수행하지 못합니다. 다만, 학습 데이터셋의 순서나 경우에 따라서 ICL 성능이 크게 흔들리는 현상이 나타나, Hyperparameter에 dependent한 것으로 나타났습니다.
...
**기존 문제** : LLM은 pretraining은 고사하고 finetuning도 너무 비용이 많이 듭니다. 또한 ICL에서 few-shot example은 비록 같은 지식을 함유하더라도 순서에 따라 [추론 결과가 바뀔 수 있어](https://arxiv.org/abs/2102.09690), 불안정하다는 risk가 존재합니다.
